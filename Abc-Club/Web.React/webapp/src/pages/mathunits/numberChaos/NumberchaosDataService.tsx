...
const NumberchaosDataservice: React.FC = () =>{
    ...
    const getUnitApiOptions = React.useMemo((): IApiOptions =>{
        return {
            serviceUrl: apiConfig.baseUrl + apiConfig.math.unitController + apiConfig.math.getunit + '/' + JSON.stringify(requestModel),
            method: 'GET'
        }
    ...
    const handleAllowDrop = React.useCallback((event: React.DragEvent<HTMLDivElement>) =>{
    ...
    if(numberChaosApi.items[0] === undefined || userData.username === undefined){
        return null
    }
